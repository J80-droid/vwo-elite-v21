...
SOORT OPDRACHT: ${game.activeModule === "rotation"
      ? "3D rotatie matching"
...
- Geef NOOIT het antwoord, stel alleen vragen`,
          ...
          <div className="grid grid-cols-4 gap-6 mb-16 px-4">
            {(
              ...
              <button
                ...
                className={`p-6 rounded-2xl border transition-all duration-300 flex flex-col items-center gap-3 group relative overflow-hidden ${selectedDifficulty === key
  ...
  return (
    <div className="w-full h-full flex flex-col bg-obsidian-950 relative font-sans select-none text-slate-200 overflow-hidden">
      ...
      <div className="flex-1 w-full p-4 md:p-6 overflow-y-auto flex flex-col items-center justify-start">
        <div className="flex flex-col gap-4 w-full max-w-7xl mx-auto min-h-0">
          {/* Instruction Header */}
          <div className="text-center space-y-1 mb-2">
            ...
            {/* Quick Stats in content area */}
            <div className="flex items-center justify-center gap-4 text-[10px] md:text-xs">
              <div className="flex items-center gap-1.5 text-blue-300 bg-blue-500/10 px-2 py-0.5 rounded-full border border-blue-500/20">
                <Trophy size={10} className="text-yellow-500" />
                <span>Lvl {game.currentLevel} | Score: {game.score}</span>
              </div>
              <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full border border-white/10 ${game.timeLeft < 5 ? "text-red-500 animate-pulse bg-red-500/10 border-red-500/20" : "text-slate-400 bg-white/5"}`}>
                <Clock size={10} />
              ...
              >
                <MonitorPlay size={12} className={soundEnabled ? "text-emerald-500" : "opacity-30"} />
              </button>
            </div>
          </div>
        </div>

        {/* NEW SPLIT LAYOUT: Options Left, Question Right */}
        <div className="flex flex-col xl:flex-row gap-6 w-full min-h-[500px] flex-1">
