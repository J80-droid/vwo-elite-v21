...
export class KimiProvider extends BaseProvider {
    readonly id: PROVIDER_ID = "kimi";
    readonly capabilities: ProviderCapabilities = {
        multimodal: false, // Kimi is primarily text-based for now (Moonshot-v1)
        tools: true,
        jsonMode: true,
        maxContext: 128000,
    };
    ...
    async generate(
        ...
        const model = options.modelId || resolveModel("kimi", "chat", options.aiConfig);
        ...
        try {
            ...
            const config = {
                ...
                apiKey: options.aiConfig?.kimiApiKey || "",
                enabled: true,
                models: { chat: model }
            };
            ...
            const durationMs = performance.now() - startTime;
            logger.error(`Kimi failed:`, error);
